...
const content = await fs.readFile("alumni.csv");
const alumni = parse(content, { bom: true });
const people = alumni.map(alumnus => ({
  company: alumnus[0],
  role: alumnus[1],
  interests: alumnus[2],
  interestToAvoid: alumnus[3],
  bio: alumnus[4],
}));

const groq = new Groq();

export async function getAlumniRating(person) {
...
//   console.log(await getAlumniRating(person));
// }